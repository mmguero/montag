...
def main():
  ...
  parser = argparse.ArgumentParser(description='e-book profanity scrubber', add_help=False, usage=f'{os.path.basename(__file__)} [options]')
  ...
  requiredNamed.add_argument('-w', '--word-list', dest='swears', metavar='<STR>', type=str, default=os.path.join(os.path.dirname(os.path.realpath(__file__)), 'swears.txt'), help='Profanity list text file (default: swears.txt)')
  ...
  with tempfile.TemporaryDirectory() as tmpDirName:
    ...
    metadataExitCode = subprocess.call(["/usr/bin/ebook-meta", args.input, "--to-opf="+metadataFileSpec], stdout=devnull, stderr=devnull)
    if (metadataExitCode != 0):
      raise subprocess.CalledProcessError(metadataExitCode, f"/usr/bin/ebook-meta {args.input} --to-opf={metadataFileSpec}")
    ...
    metadataExitCode = subprocess.call(["/usr/bin/ebook-meta", args.output, "--from-opf="+metadataFileSpec], stdout=devnull, stderr=devnull)
    if (metadataExitCode != 0):
      raise subprocess.CalledProcessError(metadataExitCode, f"/usr/bin/ebook-meta {args.output} --from-opf={metadataFileSpec}")
